...
struct MsgAlloc {
    ...
    void decrypt(uint16_t key, int i) {
        encrypt(key, i);
    }
};
...
protected:
    ...
    vector<int> vec_gmm_linenos;
    vector<int> vec_gmm_lineoffsets;

    template <typename key_type, typename mapped_type>
    void CreateInverseMap(map<key_type, mapped_type> const &_in, map<mapped_type, key_type> &_out) {
        for (auto _pair : _in) {
...
public:
    ...
    vector<int> &GetGmmLineNos() {
        return vec_gmm_linenos;
    }
    vector<int> &GetGmmLineOffsets() {
        return vec_gmm_lineoffsets;
    }
