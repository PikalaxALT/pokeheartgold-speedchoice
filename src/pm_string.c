...
static const u16 sAsciiToPMString[128] = {
    EOS,
    CHAR_QMARK,
    CHAR_QMARK,
    CHAR_QMARK,
...
String *ConvertAsciiToPmString(const char *asciiStr, HeapID heapId) {
    size_t slen = strlen(asciiStr);
    String *ret = String_New(slen + 1, heapId);
    GF_ASSERT(ret != NULL);
    for (int i = 0; i <= slen; ++i) {
        String_AddChar(ret, asciiStr[i] >= 0 ? sAsciiToPMString[asciiStr[i]] : CHAR_QMARK);
    }
    return ret;
}
